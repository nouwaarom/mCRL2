...
protected:
    ...
    vector<set<std::size_t> >
    create_occurrence_set(const std::vector<boolean_equation> &eqs, const std::size_t current_rank)
    {
...
public:
    bool run(std::vector<bool> *full_solution)
    {
        ...
        for (std::size_t i = 0; i < eqs.size(); i++) {
            ...
            std::cout << eqs[i].variable().name() << ":";
            if (is_true(eqs[i].formula())) {
                std::cout << "true";
            }
            else {
                std::cout << "false";
...
bool local_fixpoints(boolean_equation_system &b, std::vector<bool> *full_solution = nullptr)
{
    ...
    bool result = algorithm.run(full_solution);
