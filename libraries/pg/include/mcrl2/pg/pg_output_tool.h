...
#ifndef MCRL2_PG_PG_OUTPUT_TOOL_H
#define MCRL2_PG_PG_OUTPUT_TOOL_H
...
class pg_output_tool: public Tool
{
  protected:

    /// \brief The type of the pbes output format
    utilities::file_format m_pg_output_format;
    ...
    void parse_options(const utilities::command_line_parser& parser)
    {
      ...
      m_pg_output_format = utilities::file_format();
      if(parser.options.count("out"))
      {
        ...
        for (const utilities::file_format& type: types)
        {
          ...
          {
            m_pg_output_format = type;
          }
        }
        if (m_pg_output_format == utilities::file_format())
        {
      ...
      if (m_pg_output_format == utilities::file_format())
      {
        m_pg_output_format = default_output_format();
        mCRL2log(log::verbose) << "Guessing output format: " << m_pg_output_format.description()
                               << std::endl;
      }
    }
  ...
  public:
    ...
    pg_output_tool(const std::string& name,
...
#endif // MCRL2_PG_PG_OUTPUT_TOOL_H
